...
void SAMPLE_VENC_Usage(char* sPrgNm)
{
    ...
    printf("\t  6) zhouhua getFrame.\n");
    printf("\t  7) zhouhua getFrame to file.\n");
...
static void* getHandle(){
    ...
    ******************************************/
    ...
    for (i = 0; i < s32ChnTotal; i++)
    {
        ...
        VencChn = 0;// zhouhua this pstPara->VeChn[i];
...
static HI_S32 pack3mem(VENC_STREAM_S* pstStream,char* buffer,int*datalen)
{
    HI_S32 i;
    int len = 0;
    for (i = 0; i < pstStream->u32PackCount; i++)
    {
        memcpy(&(buffer[len]),pstStream->pstPack[i].pu8Addr + pstStream->pstPack[i].u32Offset,pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset);
...
static HI_S32 pack2mem(VENC_STREAM_BUF_INFO_S *pstStreamBuf, VENC_STREAM_S* pstStream,char* buffer,int* datalen)
{
    HI_U32 i,j;
    ...
    printf("__%d\n",__LINE__);
    for(i=0; i<pstStream->u32PackCount; i++)
    {printf("__%d ppp = %d\n",__LINE__,pstStream->u32PackCount);
        ...
        {printf("__%d\n",__LINE__);
            ...
            {printf("__%d\n",__LINE__);
                ...
                u64SrcPhyAddr = pstStreamBuf->u64PhyAddr[j] +
                                ...
                                (pstStreamBuf->u64PhyAddr[j] + pstStreamBuf->u64BufSize[j]));
printf("__%d\n",__LINE__);
                memcpy(&(buffer[len]), (void *)(HI_UL)u64SrcPhyAddr,pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset);
                len+=(pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset);
            ...
            {printf("__%d\n",__LINE__);
                ...
                memcpy(&(buffer[len]),(void *)(HI_UL)(pstStream->pstPack[i].u64PhyAddr + pstStream->pstPack[i].u32Offset),u32Left - pstStream->pstPack[i].u32Offset);
                len+=(u32Left - pstStream->pstPack[i].u32Offset);
                printf("__%d\n",__LINE__);
                // s32Ret = fwrite((void *)(HI_UL)(pstStream->pstPack[i].u64PhyAddr + pstStream->pstPack[i].u32Offset),u32Left - pstStream->pstPack[i].u32Offset, 1, pFd);
                memcpy(&(buffer[len]),(void *)(HI_UL)pstStreamBuf->u64PhyAddr[j],pstStream->pstPack[i].u32Len - u32Left);
        ...
        {
            printf("__%d %d   %d oof=%d\n",__LINE__,len,pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset,pstStream->pstPack[i].u32Offset);
            /* physical address retrace does not happen */
            //memcpy(&(buffer[len]),(void *)(HI_UL)(pstStream->pstPack[i].u64PhyAddr + pstStream->pstPack[i].u32Offset),pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset);
            printf("__%d\n",__LINE__);
            len+=(pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset);
            printf("__%d\n",__LINE__);
...
int venc_init(int resolve)
{
    ...
    VPSS_GRP        VpssGrp        = 0;
    VPSS_CHN        VpssChn[2]     = {0,1};
    HI_BOOL         abChnEnable[VPSS_MAX_PHY_CHN_NUM] = {1,0,0};//zhouhua this. use chn 0 only
    SAMPLE_VPSS_CHN_ATTR_S stParam;
    SAMPLE_VB_ATTR_S commVbAttr;
    ...
    ******************************************/
    ...
    for (int j = 0; j < commVbAttr.validNum; j++)
    {
        printf("000000000000%s,%d,stVbConf.astCommPool[%d].u64BlkSize = %lld, blkSize = %d\n",__func__,__LINE__,j,commVbAttr.blkSize[j],commVbAttr.blkCnt[j]);
    }
    ...
    ******************************************/
    ...
    enGopMode = VENC_GOPMODE_NORMALP;//VENC_GOPMODE_SMARTP;//SAMPLE_VENC_GetGopMode();
    s32Ret = SAMPLE_COMM_VENC_GetGopAttr(enGopMode,&stGopAttr);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Venc Get GopAttr for %#x!\n", s32Ret);
...
int venc_getFrame(char* buffer,int *datalen)
{
    HI_S32 i;
    HI_S32 s32ChnTotal = 1;
            ...
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    VencChn = 0;//pstPara->VeChn[i]; zhouhua this
                    ...
                    *******************************************************/
                    pack3mem(&stStream,buffer,datalen);
                    //pack2mem(&stStreamBufInfo[i], &stStream,buffer,datalen);
...
HI_S32 zhouhua(){
    int len;
    int ret;
    char* mem = (char*)malloc(102400);
    if(mem == NULL)printf("malloc fail\n\n\n\n");
    venc_init(1);
    while(1){
        ret = venc_getFrame(mem,&len);
        printf("ret = %d len= %d\n",ret,len);
    }
}
HI_S32 zhouhua2(){
...
HI_S32 SAMPLE_VENC_ROIBG(void)
{
    ...
    for(i=0; i<s32ChnNum; i++)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("SAMPLE_COMM_SYS_GetPicSize failed! laallalallalal\n");
            return s32Ret;
        }
    }
